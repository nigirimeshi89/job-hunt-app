                        ...
                        {showNotifications && (
                            <div className="absolute right-0 top-10 w-80 bg-white rounded-xl shadow-xl border border-gray-200 overflow-hidden z-50 dark:bg-slate-800 dark:border-slate-600">
                                ...
                                <div className="max-h-64 overflow-y-auto">
                                    {notifications.length === 0 ? <p className="text-center text-gray-400 text-sm py-4">通知はありません</p> : notifications.map(n => (
                                        <div key={n.id} onClick={() => onReadNotification(n.id)} className={`p-3 text-sm border-b border-gray-100 hover:bg-blue-50 cursor-pointer transition-colors dark:border-slate-700 dark:hover:bg-slate-700 ${n.is_read ? 'opacity-50' : 'bg-blue-50/30 font-bold dark:bg-slate-700/50'}`}>
